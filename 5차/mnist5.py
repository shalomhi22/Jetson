...
if __name__ == '__main__' :
    ...
    cnt = 0; t = 0
    while cnt < 6000 :
        if cnt == 0 or cnt == 5000 or cnt == 5500 :
            f = open('sub-'+types[t]+'-annotations-bbox.csv', 'w', encoding='utf-8', newline='')
            wf = csv.writer(f)
            wf.writerow(['ImageID', 'Source', 'LabelName', 'Confidence' , 'XMin', 'XMax', 'YMin', 'YMax', 'IsOccluded', 'IsTruncated', 'IsGroupOf', 'IsDepiction', 'IsInside', 'id', 'ClassName'])
        ...
        for i in range(num) :
            wf.writerow(['image'+str(cnt), 'freeform', classes[target_list[i]], 1 , bR_arr[i][0], bR_arr[i][1], bR_arr[i][2], bR_arr[i][3], 0, 0, 0, 0, 0, classes[target_list[i]], classes[target_list[i]].split('0')[1].capitalize()])
